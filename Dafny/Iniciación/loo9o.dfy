...
method testPar(){
	assert esPar(2);
	//assert esPar(5); Falla porque 5 no es par
	//assert esPar(-2); Falla porque no es positivo
...
{forall u::0<=u<|s| ==> s[u]>=0} //Para todo u desde 0 hasta tamaño de s, s[u] es mayor que 0

method TestPositivos()
{
	assert positivos([1,2,3]);
	//assert positivos([1,-3,7,8]); //Falla porque el -3 es negativo
...
method TestIguales() 
{
	...
	s := [1,1,1,1,1];
	assert iguales(s);
	//assert iguales([1,1,1,1,2]); //Falla el 2 no es igual
...
method TestDistintosDe() 
{
	...
	//assert distintosDe([3,4,3,6],3); // Este assert debe fallar - falla por el 3
...
method TestDistintos() 
{
	...
	s := [1,2];
	assert distintos(s);
	//assert distintos([3,4,3,6]); //Falla por el 3
	//assert distintos([6,3,3,1,5]); //Falla por el 3   
...
method TestContiguosDistintos() 
{
	...
	//assert contiguosDistintos([6,3,3,1,5]); //Falla
	var v:array<int> := new int[4];
...
method TestOrdenado()
{
    ...
    //assert ordenado([3,6,1,9,10]); //Falla
}
...
method TestOrdenadoEstricto()
{
    ...
    //assert ordenadoEstricto([3,10,10,28]); //Falla porque es estrictamente creciente 10=10
...
predicate esPrimerCero(s : seq<int>, k : int) 
	requires k <= |s|
{
   	forall x :: 0 <= x < k < |s| ==> (s[x] != 0 && s[k] == 0) || (s[x] !=0 && k == |s|)
}

method TestEsPrimerCero() 
{  	var s : seq<int>;
    s := [1,0,1,5,1];
    assert esPrimerCero(s,5);
	assert esPrimerCero(s,1);
	s:= [1,0,4,5];
    //assert esPrimerCero(s,3);
...
//1. Este predicado determina si una secuencia es permutacion de otra
